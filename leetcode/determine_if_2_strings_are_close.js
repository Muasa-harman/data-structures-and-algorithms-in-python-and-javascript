...
var closeStrings = function (word1, word2) {
  ...
  // Step 2: Helper function to count character frequencies
  const countChars = (word) => {
    let count = new Array(26).fill(0);
    for (let char of word) {
      count[char.charCodeAt(0) - "a".charCodeAt(0)]++;
  ...
  // Step 3: Check if both strings have the same set of unique characters
  const hasSameUniqueChars = (count1, count2) => {
    for (let i = 0; i < 26; i++) {
      if (
  ...
  if (!hasSameUniqueChars(count1, count2)) {
    return false;
  }

  // Step 4: Sort frequency counts and compare
  count1.sort((a, b) => a - b);
  count2.sort((a, b) => a - b);
